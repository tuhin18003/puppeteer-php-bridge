...
namespace PuppetBridge;

use PuppetBridge\Exceptions\InvalidArgumentException;
use PuppetBridge\Exceptions\PuppeteerException;
use PuppetBridge\Exceptions\TemplateNotFoundException;
use PuppetBridge\Image\Types;
use PuppetBridge\Image\Types\Type;
use PuppetBridge\Injection\LocalPath;
use PuppetBridge\Injection\Url;
use PuppetBridge\Location\Jobs;
use PuppetBridge\Location\Output;

/**
 * Class Capture
 *
 * @package PuppetBridge
...
class Capture {
    ...
    /**
     * Jobs directory, directory for temporary files to be written and executed with puppeteer
     *
     * @var Jobs
     */
    public $jobs;
    ...
    public function save($imageLocation, $deleteFileIfExists = true)
    {
        ...
        $data = array(
            ...
            'type'          => $this->getImageType()->getFormat()
        );
        ...
        $command = sprintf("node %s %s",  $this->getOptionsString(), $jobPath);
        ...
        if ($returnCode !== 0 ) {
            throw new PuppeteerException($output);
        }
    ...
    /**
     * Sets the options which will be passed to puppeteer
     *
     * @param array $options
     *
     * @return $this
